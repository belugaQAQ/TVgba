...
class GameListMaterialActivity : ComponentActivity() {
    ...
    private fun setupUI() {
        val romDirectory = File("/rom").apply { if (!exists()) mkdirs() }
        val documentfile = DocumentFile.fromFile(romDirectory)
        val coverfilefolder = documentfile.findFile("gbacovers")
        setContent { CustomCircularProgressBar() }
